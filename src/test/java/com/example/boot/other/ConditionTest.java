    ...
    public static void main(String[] args) throws Exception {
        ...
        final Thread thread1 = new Thread("Thread 1 ") {
            ...
            public void run() {
                ...
                try {
                    Thread.sleep(2 * 1000);
                    log.info(Thread.currentThread().getName() + " 停止运行, 等待一个 signal ");
                    condition.await(); // 调用 condition.await 进行释放锁, 将当前节点封装成一个 Node 放入 Condition Queue 里面, 等待唤醒
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
                log.info(Thread.currentThread().getName() + " 获取一个 signal, 继续执行 ");
                lock.unlock(); // 释放锁
            }
        };
        ...
        Thread thread2 = new Thread("Thread 2 ") {
            ...
            public void run() {
                ...
                log.info(Thread.currentThread().getName() + " 发送一个 signal ");
                log.info(Thread.currentThread().getName() + " 发送 signal 结束");
