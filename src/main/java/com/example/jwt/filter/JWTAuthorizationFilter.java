...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        // 如果请求头中没有Authorization信息则直接放行了(此demo只拦截了部分路径)
        if (tokenHeader == null || !tokenHeader.startsWith(JwtTokenUtils.TOKEN_PREFIX)) {
            chain.doFilter(request, response);
            return;
        }
        boolean before = JwtTokenUtils.isExpiration(tokenHeader.replace("Bearer ", ""));
        if (!before) {
            System.err.println("token 已过期");
            ResponseHelper.setResponse(response, JSON.toJSONString(Response.auth("用户尚未登录或登录信息过期")));
            return;
        }
        // TODO: 2019/3/11 暂无法主动设置token的过期时间，只能依靠数据库或redis。存token及对应的过期时间
